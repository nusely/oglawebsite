      ...
      <div className="relative overflow-hidden aspect-square">
        ...
        {brand && (
          <div 
            ...
            style={{ backgroundColor: brandColors?.primary || brand.brandColors.primary }}
          >
            {brand.name}
          </div>
        )}
        
        {/* Featured Badge - Uses Brand Color */}
        {product.isFeatured && (
          <div 
            className="absolute top-2 right-2 sm:top-3 sm:right-3 text-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-xs font-medium"
            style={{ backgroundColor: brandColors?.primary || brand?.brandColors?.primary || '#b5a033' }}
          >
            Featured
          </div>
        )}
      ...
      <div className="p-3 sm:p-4 flex-1 flex flex-col">
        ...
        <Link to={`/products/${product.slug}`}>
          <h3 
            ...
            onMouseEnter={(e) => {
              e.target.style.color = brandColors?.primary || brand?.brandColors?.primary || '#b5a033';
            }}
            onMouseLeave={(e) => {
