  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      if (decoded) {
        ...
        const user = await query('SELECT id, firstName, lastName, email, role FROM users WHERE id = ?', [decoded.userId]);
        if (user.length > 0) {
          req.user = user[0];
...
router.post('/', optionalAuth, [
  ...
  try {
    ...
    const product = await query('SELECT id, name FROM products WHERE id = ? AND isActive = 1', [productId]);
    if (product.length === 0) {
    ...
    console.log('Inserting review with data:', { productId, userId, rating, title, comment });
...
router.put('/:reviewId', authenticateToken, [
  ...
  try {
    ...
    const existingReview = await query('SELECT * FROM reviews WHERE id = ?', [reviewId]);
    if (existingReview.length === 0) {
...
router.delete('/:reviewId', authenticateToken, async (req, res) => {
  try {
    ...
    await run('UPDATE reviews SET isActive = 0, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [reviewId]);
...
router.patch('/:reviewId/toggle-status', authenticateToken, async (req, res) => {
  try {
    ...
    await run('UPDATE reviews SET isActive = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [newStatus, reviewId]);

    res.json({ 
      message: `Review ${newStatus ? 'activated' : 'deactivated'} successfully`,
      isActive: newStatus
    });
