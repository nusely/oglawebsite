...
router.post('/', requireSuperAdmin, async (req, res) => {
  try {
    ...
    `, [firstName, lastName, email, hashedPassword, role, companyName, companyType, companyRole, phone]);
    ...
    await logActivity(req.user.id, 'admin_user_created', 'user', result.insertId, 'Admin created new user', req, {
...
router.get('/', [
  ...
  validatorQuery('role').optional().isIn(['customer', 'admin', 'super_admin']).withMessage('Valid role is required'),
  validatorQuery('isActive').optional().isBoolean().withMessage('Active status must be a boolean')
], async (req, res) => {
  try {
    ...
    if (search) {
      whereConditions.push('(firstName LIKE ? OR lastName LIKE ? OR email LIKE ? OR companyName LIKE ?)');
    ...
    // Format users (no address field to parse)
...
router.post('/bulk/delete', authenticateToken, async (req, res) => {
  try {
    ...
    await logActivity(req.user.id, 'bulk_users_deleted', 'user', null, `Admin deleted ${userIds.length} users`, req, {
      userIds: userIds,
...
router.post('/bulk/update-status', authenticateToken, async (req, res) => {
  try {
    ...
    await logActivity(req.user.id, 'bulk_users_status_updated', 'user', null, `Admin updated status for ${userIds.length} users`, req, {
      userIds: userIds,
...
router.get('/:id/requests', requireSuperAdmin, async (req, res) => {
  try {
    ...
    // Check if user exists
    const user = await query('SELECT id, firstName, lastName, email FROM users WHERE id = ?', [id]);
    if (user.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'User not found'
...
router.get('/export/csv', authenticateToken, async (req, res) => {
  try {
    ...
    const csvHeader = 'ID,First Name,Last Name,Email,Role,Status,Total Requests,Created,Updated\n';
    const csvRows = users.map(user => 
      `${user.id},"${user.firstName || ''}","${user.lastName || ''}","${user.email || ''}","${user.role || ''}",${user.isActive ? 'Active' : 'Inactive'},${user.totalRequests || 0},"${user.createdAt || ''}","${user.updatedAt || ''}"`
    ).join('\n');
