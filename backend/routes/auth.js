...
const pickUserPublic = (u) => {
  if (!u) return u;
  const { password, resetPasswordToken, resetPasswordExpires, emailVerificationToken, emailVerificationExpires, ...rest } = u;
  return rest;
};
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) return res.status(400).json(buildValidationErrorResponse(errors));

      const { firstName, lastName, email, phone, password, companyName, companyType, companyRole, address } = req.body;
      ...
      if (!userId) return res.status(500).json({ success: false, message: 'Failed to create user account' });

      // JWTs
      const token = jwt.sign({ userId }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '7d' });
      const verificationToken = jwt.sign({ userId, type: 'email_verification' }, process.env.JWT_SECRET, { expiresIn: '24h' });

      // Store verification token (SQLite syntax)
      await query(
      ...
      return res.status(201).json({
        ...
        data: { user: { id: userId, firstName, lastName, email, companyName, role: 'customer' }, token },
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (!user.emailVerified && user.role !== 'super_admin') {
        await logActivity(user.id, 'login_failed', 'user', user.id, 'Email not verified', req);
      ...
      await query('UPDATE users SET lastLoginAt = CURRENT_TIMESTAMP WHERE id = ?', [user.id]);
      ...
      return res.json({ success: true, message: 'Login successful', data: { user: pickUserPublic(user), token } });
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const users = await query('SELECT id, firstName FROM users WHERE email = ? AND isActive = 1', [email]);
      ...
      await logActivity(user.id, 'password_reset_requested', 'user', user.id, 'User requested password reset', req, { email });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (users.length === 0) {
        ...
        try {
          ...
          if (userCheck.length > 0) {
            ...
            console.log('   - Token matches:', u.emailVerificationToken === token);
            console.log('   - Token expires:', u.emailVerificationExpires);
            console.log('   - Current time:', u.now);
            console.log('   - Token expired:', u.emailVerificationExpires <= u.now);
          }
        } catch (_) {}
      ...
      await query('UPDATE users SET emailVerified = 1, emailVerificationToken = NULL, emailVerificationExpires = NULL WHERE id = ?', [decoded.userId]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const users = await query('SELECT id, firstName, emailVerified FROM users WHERE email = ? AND isActive = 1', [email]);
      if (users.length === 0) return res.status(404).json({ success: false, message: 'User not found' });

      const user = users[0];
      if (user.emailVerified) return res.status(400).json({ success: false, message: 'Email is already verified' });

      const verificationToken = jwt.sign({ userId: user.id, type: 'email_verification' }, process.env.JWT_SECRET, { expiresIn: '24h' });
...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const user = users[0];
    if (user.address) {
      try { user.address = JSON.parse(user.address); } catch (_) {}
    }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const allowed = ['firstName', 'lastName', 'phone', 'companyName', 'companyType', 'companyRole', 'address'];
